codeunit 50190 "AWS Lambda Invoker"
{
    ...
    local procedure CalcCombinedPct(VolPct: Decimal; AddPct: Decimal): Decimal
    var
        V: Decimal;
        A: Decimal;
        R: Decimal;
    ...
    local procedure GetSellToEmail(SalesHeader: Record "Sales Header"): Text[250]
    var
    ...
    local procedure GetSellToContactName(SalesHeader: Record "Sales Header"): Text[250]
    var
        Cont: Record Contact;
        FullName: Text[250];
    begin
    ...
    local procedure GetSellToCountryCode(SalesHeader: Record "Sales Header"): Code[10]
    var
        Cust: Record Customer;
        Cont: Record Contact;
        Code: Code[10];
    ...
    local procedure GetCountryName(CountryCode: Code[10]): Text[100]
    var
    ...
    local procedure GetSellToLanguageCode(SalesHeader: Record "Sales Header"): Code[10]
    var
        Cust: Record Customer;
        Cont: Record Contact;
        Code: Code[10];
...
codeunit 50194 "CRM Sale Won Subscriber"
{
    [EventSubscriber(
        ObjectType::Codeunit,
        ...
        false,
        false)]
    local procedure OnAfterCreateSalesHeader(
    ...
    begin
        SendSaleWonToCRM(
            SalesHeader."No.",
            SalesOrderHeader."No.",
            SalesOrderHeader.SystemCreatedAt
        );
    end;


    local procedure SendSaleWonToCRM(
        QuoteNo: Code[20];
        SalesOrderNo: Code[20];
        OrderCreatedAt: DateTime)
    var
        LambdaSetup: Record "Lambda Setup";
        Client: HttpClient;
        Content: HttpContent;
    ...
    begin
        LambdaSetup.Reset();
        LambdaSetup.SetRange(Enabled, true);

        if not LambdaSetup.FindFirst() then
            Error('No hay ninguna configuración Lambda activa.');

        if LambdaSetup."Endpoint URL" = '' then
            Error('La configuración activa no tiene Endpoint URL.');

        // Construimos la URL completa
        Endpoint :=
            LambdaSetup."Endpoint URL" +
            '/venta_ganada';

        JsonBody.Add('quote_number', QuoteNo);
        JsonBody.Add('pedido_venta', SalesOrderNo);
        ...
        JsonBody.Add('database', LambdaSetup."Database Name");
        ...
        if LambdaSetup."API Key" <> '' then
            Client.DefaultRequestHeaders.Add(
                'X-API-Key',
                LambdaSetup."API Key"
            );

        if LambdaSetup."Bearer Token" <> '' then
            Client.DefaultRequestHeaders.Add(
                'Authorization',
                StrSubstNo(
                    'Bearer %1',
                    LambdaSetup."Bearer Token"
                )
            );
        ...
        if not Client.Post(
            ...
            Error(
                'No se ha podido conectar con el CRM. URL: %1',
                Endpoint
            );
        ...
        if Response.HttpStatusCode() = 404 then begin
            Message(
                'El pedido %1 se ha creado, pero la oferta %2 no existe en el CRM.',
                SalesOrderNo,
                QuoteNo
            );
            exit;
        ...
        Message(
            'CRM actualizado. Oferta %1 - Pedido %2',
            QuoteNo,
            SalesOrderNo
        );
    end;
