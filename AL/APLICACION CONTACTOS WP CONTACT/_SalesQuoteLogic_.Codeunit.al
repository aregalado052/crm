codeunit 50135 "SalesQuoteLogic"
{
    ...
    procedure CreateWebContact(Name: Text[100]; Email: Text[100]; CustomerTemplateName: Text[100]; CountryRegionCode: Code[10]; CodIdioma: Code[10]): Code[20]
    var
    ...
    procedure CreateWebQuote(Name: Text[100]; Email: Text[100]; CustomerTemplateName: Code[40]; CountryRegionCode: Code[10]): Code[20]
    var
        Contact: Record Contact;
    ...
    procedure CreateQuote(Name: Text[100]; Email: Text[100]; CustomerTemplateName: Code[40]; CountryRegionCode: Code[10]; CodIdioma: Code[10]): Code[20]
    var
        ContactNo: Code[20];
    begin
    ...
    procedure UpdateContactFromQuote(
        BillToName: Text[100];
        QuoteNo: Code[20];
        Address: Text[100];
        Address2: Text[50];
    ...
    begin
        ...
        if ContactNo = '' then
            Error('La oferta %1 no tiene Sell-to Contact No.', QuoteNo);

        if not Contact.Get(ContactNo) then
            Error('Contacto no encontrado: %1', ContactNo);


        if BillToName <> '' then SalesHeader."Bill-to Name" := BillToName;
        if Address <> '' then Contact.Address := Address;
        if Address2 <> '' then Contact."Address 2" := Address2;
        if PostCode <> '' then Contact."Post Code" := PostCode;
        if City <> '' then Contact.City := City;

        if VATRegNo <> '' then
            Contact."VAT Registration No." := VATRegNo;

        if ForeignRegNo <> '' then
            Contact."Foreign Registration No." := ForeignRegNo;

        Contact.Modify(true);

        // --- OFERTA / SALES HEADER (para que la proforma “quote” se vea con datos) ---
        if Address <> '' then SalesHeader."Sell-to Address" := Address;
        if Address2 <> '' then SalesHeader."Sell-to Address 2" := Address2;
        if PostCode <> '' then SalesHeader."Sell-to Post Code" := PostCode;
        if City <> '' then SalesHeader."Sell-to City" := City;

        // Si tu layout/proforma usa Bill-to:
        if Address <> '' then SalesHeader."Bill-to Address" := Address;
        if Address2 <> '' then SalesHeader."Bill-to Address 2" := Address2;
        if PostCode <> '' then SalesHeader."Bill-to Post Code" := PostCode;
        if City <> '' then SalesHeader."Bill-to City" := City;
