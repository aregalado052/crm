...
def actualizar_crm_venta_ganada(database, quote_number, pedido_venta, fecha_pedido_venta):
    ...
    try:
        ...
        connection.commit()
        print(f"Filas actualizadas en lead_forms para quote_number {quote_number}: {updated}")
        if updated == 0:
            return {
                "statusCode": 404,
                "headers": {
                ...
                "body": json.dumps({
                    "ok": False,
                    "message": f"No se encontró la oferta {quote_number} en el CRM"
                })
            }
        ...
        return {
            ...
            "body": json.dumps({
                "ok": True,
                "message": "Oferta actualizada como Ganada",
                "quote_number": quote_number,
                "pedido_venta": pedido_venta,
                "fecha_pedido_venta": fecha_pedido_venta
            }, default=str)
        }
        ...
        print("ERROR actualizando venta ganada:", e)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if route_key == "POST /venta_ganada":
            payload = _parse_json_body(event) or {}

            quote_number = (payload.get("quote_number") or "").strip()
            pedido_venta = (payload.get("pedido_venta") or "").strip()
            fecha_pedido_venta = (payload.get("fecha_pedido_venta") or "").strip()
            database = (payload.get("database") or "").strip()

            if not quote_number or not pedido_venta or not fecha_pedido_venta or not database:
                return {
                    "statusCode": 400,
                    "headers": {
                        "Content-Type": "application/json"
            ...
            return actualizar_crm_venta_ganada(
                database,
                quote_number,
                pedido_venta,
                fecha_pedido_venta
...
if __name__ == "__main__":
    ...
    @app.route("/venta_ganada", methods=["POST"])
    def venta_ganada():
        ...
        pedido_venta = data.get("pedido_venta")
        fecha_pedido_venta = data.get("fecha_pedido_venta")
        ...
        result = actualizar_crm_venta_ganada(
            database,
            quote_number,
            pedido_venta,
            fecha_pedido_venta
